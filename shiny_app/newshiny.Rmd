...
plot_ly(labels = ~activity, values = ~number_of_ture) %>%
  add_pie(hole = 0.6) %>%
  layout(title = "Distribution of Activity In One Day",  showlegend = F,
...
squrriel_plot_3 %>%
  filter(
      age_group == input[["Age Group"]]) %>% 
 plot_ly(x = ~Day, y = ~interaction, type = 'bar', name = 'Primary Product', marker = list(color = 'rgb(149,130,189)')) %>%
  add_trace(y = ~noninteraction, name = 'Secondary Product', marker = list(color = 'rgb(100,100,100)')) %>%
